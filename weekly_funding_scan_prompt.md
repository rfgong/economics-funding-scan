...
Read the current repository versions of `funding_roster.json` and `funding_state.json` first. Treat them as working memory; do not reconstruct missing state from chat memory.
...
1. If an `open` program's operative deadline has passed, force a recheck regardless of `recheck_after`; if no new cycle exists, update its status.
2. For an open program, `recheck_after` may never exceed its operative deadline.
3. Every program actually rechecked receives a new `recheck_after`, even if nothing changed.
...
Default `recheck_after` advancement when no better timing is known:
...
- Scan quarterly seed pages on the first successful run in January, April, July, and October if `funding_state.discovery_checks.quarterly_last_run` is not already in the current quarter; check at most **8** quarterly pages and then update that state field.
...
If a newly discovered program reaches the main list or ALTERNATES, retain temporary metadata in state and propose a durable roster entry under maintenance.
...
Create `status: planned` only when the user has stated an intention to apply or a `THIS WEEK` action explicitly commits to application preparation. Do not create it merely because an opportunity was surfaced or investigated.

Keep public state minimal and non-sensitive. Never store proposal text, letters, credentials, private correspondence, reviewer feedback, or sensitive personal information.
...
Do not show implementation details, state JSON, roster JSON, or debugging in the researcher-facing report.

## 7. Maintenance output

Only when `funding_state.json` or `funding_roster.json` actually needs updating, append a clearly separated `MAINTENANCE` section **after** the researcher-facing report.

- For state changes, emit the **complete replacement** `funding_state.json` with updated `as_of`, rechecks, discovery checks, newly discovered programs, and justified application-history changes.
- For durable roster changes, emit only the proposed changed/new roster object(s), not the full roster.
- If nothing changed, omit `MAINTENANCE` entirely.
